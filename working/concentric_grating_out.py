...
from panda3d.core import Texture, CardMaker, TextureStage, SamplerState, ClockObject
...
def concentric_circles(tex_size = 512, circ_center = (512//2, 512//2), 
                       ...
                       period = 50, phase_shift = 0):
    concentric_grating = bg_color*np.ones((tex_size, tex_size), dtype = np.uint8)
    circ_radii = np.arange(tex_size+phase_shift, 0, -period)
    #print(circ_radii)
...
cc_tex = concentric_circles(tex_size = tex_size, circ_center = circ_center, 
                       ...
                       period = period, phase_shift = phase_shift)
#plt.imshow(cc_tex, cmap = 'gray')

#%%
image_dims = 512
phase_shift = 1
phase_vals = np.arange(0, 60, phase_shift); 
...
for phase_ind in range(num_phases):
    grating_cube[phase_ind,:, :] = concentric_circles(tex_size = image_dims, circ_center = circ_center, 
                       bg_color = bg_color, circ_color = circ_color, circ_thickness = circ_thickness,
                       period = period, phase_shift = phase_vals[phase_ind])
...
class CubeShow(ShowBase):
    def __init__(self, texture_cube, window_size = 512, texture_size = 512):
        ...
        #ShowBaseGlobal.globalClock.setFrameRate(60)  #can lock this at 
