...
class BinocularDrift(ShowBase):
    ...
    def __init__(self, texture_array, stim_angles = (0, 0), mask_angle = 0, position = (0,0), 
                 velocities = (0,0), band_radius = 3, window_size = 512, texture_size = 512, 
                 bgcolor = (0, 0, 0, 1)):
        super().__init__()
        ...
        if self.left_velocity != 0 and self.right_velocity != 0:
            self.taskMgr.add(self.texture_update, "texture_shifter")
...
if __name__ == '__main__':
    stim_params = {'spatial_freq': 20, 'stim_angles': (0, 90), 'velocities': (.05, .01), 
                   'position': (0.2, 0), 'band_radius': 2}
    mask_angle = 0
    texture_size = 512
    window_size = 512  
    bgcolor = (0, 0, 0, 1)
